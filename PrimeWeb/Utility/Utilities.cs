...
namespace PrimeWeb
{
    ...
    {
        public static int Search(byte[] src, byte[] pattern, int maxpos = int.MaxValue)
        {
            int maxFirstCharSlot = src.Length - pattern.Length + 1;

            maxFirstCharSlot = maxpos < maxFirstCharSlot ? maxpos : maxFirstCharSlot;
    ...
    //EncoderParameters paramList = bitmap1.GetEncoderParameterList(pngCodec.Clsid);
        ...
        public static Bitmap ResizeImage(Image image, int maxWidth, int maxHeight, bool returnIncludesCanvas=true, PrimeParameters settings = null)
        {
            ...
            try
            {
                if (settings != null)
                {
                    // TODO: Convert from platform specific to Web image dithering
                    /*
